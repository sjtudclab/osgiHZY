...
import com.em.bdmp.common.kettle.JobStatus;
...
public class KettleJobTask implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			sb.append("[正常] 开始调用......\n");
			// 开始运行Job节点
			String service = HttpKettle.CONTEXT_RUNTJOB + "/?job=" + URLEncoder.encode(jobinfo.getProgramPath() + "/", "UTF-8")
					+ URLEncoder.encode(jobinfo.getProgramName(), "UTF-8") + "&level=Basic";
			String responseBody = HttpKettle .execService(service, jobServer);
			...
			// 检查Job节点运行状态
			service = HttpKettle.CONTEXT_JOBSTATUS + "/?name=" + URLEncoder.encode(jobinfo.getProgramName(), "UTF-8") + "&id=" + carteObjectId + "&xml=Y";
			boolean falg = true;
			do{
				responseBody = HttpKettle.execService(service, jobServer);
				JobStatus jobStatus = JobStatus.fromXMLString(responseBody);
				...
				if("Finished".equalsIgnoreCase(jobStatus.getStatusDescription())){
					service = HttpKettle.CONTEXT_REMOVEJOB + "/?name=" + URLEncoder.encode(jobinfo.getProgramName(), "UTF-8") + "&id=" + carteObjectId + "&xml=Y";
				...
				if("Stop".equalsIgnoreCase(jobStatus.getStatusDescription()) || jobStatus.getStatusDescription().contains("error")){
					service = HttpKettle.CONTEXT_REMOVEJOB + "/?name=" + URLEncoder.encode(jobinfo.getProgramName(), "UTF-8") + "&id=" + carteObjectId + "&xml=Y";
					HttpKettle.execService(service, jobServer);
					String errorinfo = "ERROR信息 ：" + jobStatus.getErrorDescription() + "\n 日志内容(" + jobStatus.getFirstLoggingLineNr() + "-" + jobStatus.getLastLoggingLineNr() + ")： " + jobStatus.getLoggingString();
